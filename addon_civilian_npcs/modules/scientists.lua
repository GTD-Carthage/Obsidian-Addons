...
function SCIENTISTS_MODULE.get_levels(self)
...
function SCIENTISTS_MODULE.end_level()

  if LEVEL.prebuilt then return end

  if PARAM.bool_scientists == 1 then
    SCIENTISTS_MODULE.add_scientist()
  end

end

function SCIENTISTS_MODULE.add_scientist()
  ...
  for _,A in pairs(LEVEL.areas) do
    if (A.mode and A.mode == "floor") then
      for _,S in pairs(A.seeds) do
		...
		-- Only allow spawning in tech levels
        ...
        if rand.odds(spawn_odds) then
          ...
          local item = SCIENTISTS_MODULE.ACTORS[choice]
          local cluster
          local count = 1
          ...
          for i = 1, count do
            ...
            event_thing.id = SCIENTISTS_MODULE.ACTORS[choice].id
...
function SCIENTISTS_MODULE.all_done()

  if PARAM.bool_scientists == 1 then
    SCRIPTS.scientists_SNDINFO = SCIENTISTS_MODULE.SNDINFO
    SCRIPTS.scientists_dec = SCIENTISTS_MODULE.DEC
...
OB_MODULES["SCIENTISTS_MODULE"] =
{

  name = "SCIENTISTS_MODULE",

  label = _("ZDoom: Rescuable Scientists NPCs"),
...
--  game = "doomish",
  ...
  hooks =
  {
    get_levels = SCIENTISTS_MODULE.get_levels,
    end_level = SCIENTISTS_MODULE.end_level,
    all_done = SCIENTISTS_MODULE.all_done
  },
